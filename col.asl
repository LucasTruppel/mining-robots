...
// Retorna para posição antes ser chamado para ajudar na coleta
+!move_somewhere : prev_pos(X1,Y1)
   <- -checking_cells;
      .print("move to prev_pos X=", X1, " Y=", Y1);
      !go_cord(X1,Y1);
      -prev_pos(X1,Y1);
      +checking_cells;
      !check_for_resources.

// Se não tem posição anterior, então move para a próxima célula
+!move_somewhere : not prev_pos(_,_)
   <- move_to(next_cell).
